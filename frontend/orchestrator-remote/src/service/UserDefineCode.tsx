 ...
 * @returns
 */
const getUserDefinedCodeByGroup = async (
  indexModule: string,
  group: string
) => {
  ...
  if (iRootData) {
    try {
      ...
      console.error("Error al filtrar los codigos -> Error:", error);
    }
  }
...
const refreshUserDefinedCode = async (indexModule: string) => {
  await deleteDataById(STORE, STORE.ddl.userDefinedCode.name, indexModule).then(
    async () => {
...
export {
  getUserDefinedCodes,
  getUserDefinedCodeByGroup,
  refreshUserDefinedCode,
};
