...
import { QueryFormMenu } from "./QueryFormMenu";
import { tableQueryMenu } from "./structures/Presentations";
import { IRowDataMenu } from "./structures/Types";

/**
 * Tabla de información del sistema.
 */
const QueryMenu = ({ onEditRow, onSeeRow, onCreateRow }: IQueryProps) => {
  ...
  useEffect(() => {
    const initializeStructure = async () => {
      ...
      setApiUrl(
        parameter.valueText01 + Menus.MENU_ENDPOINT + "/paginated"
      );
    };
   ...
   */
  const configureMenuTableActions = (
    tableFormat: IPresentationTable,
    onEditRow?: (row: IRowDataMenu) => void,
    onSeeRow?: (row: IRowDataMenu) => void
  ) => {
    // Configurar acción para ver detalle
    if (tableFormat.items[1]) {
      tableFormat.items[1].onAction = {
        onAction: (row: IRowDataMenu) => {
          if (onSeeRow) {
            onSeeRow(row);
          }
        },
 ...
 /**
  ...
  const MenuTable = () => {
    return (
      <Tooltip content={t("modules."+Menus.Menu+".add")} side="left">
  ...
  return (
    <GenericQuery<IRowDataMenu>
      QueryForm={QueryFormMenu}
      getTablePresentation={tableQueryMenu}
      initialParameters={{
        name: "",
      }}
      configureTableActions={configureMenuTableActions}
      apiUrl={apiUrl}
      token={token}
