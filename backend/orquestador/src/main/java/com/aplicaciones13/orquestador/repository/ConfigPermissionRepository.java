...
public interface ConfigPermissionRepository extends JpaRepository<ConfigPermission, Long> {
    ...
    @Query(value = "SELECT "
            ...
            + "m.id_menu AS menu_id_menu, m.id_module AS menu_id_module, m.type AS menu_type, m.index AS menu_index, m.name AS menu_name, m.task_flow AS menu_task_flow, m.status AS menu_status, m.orden AS menu_orden, m.icon AS menu_icon, "
            + "mo.id_module AS module_id_module, mo.index AS module_index, mo.name AS module_name, mo.context AS module_context, mo.status AS module_status, mo.uuid AS module_uuid, mo.icon AS module_icon "
            + "FROM rol_user ru, "
            + "permission p, "
            + "menu m, "
            + "module mo "
